...
new.knn.imputed<-subset(new.knn.data , select = RowNumber : Exited)     
...
confusionMatrix(under$Exited,pred.under)  #Accuracy : 0.7286
confusionMatrix(both$Exited,pred.both)  # Accuracy : 0.7439 
...
confusionMatrix(train$Exited,pred.1)          #Accuracy : 0.7991 
confusionMatrix(over$Exited,pred.1.over)     #Accuracy : 0.73 
confusionMatrix(under$Exited,pred.1.under)  #Accuracy : 0.7286
confusionMatrix(both$Exited,pred.1.both)  # Accuracy : 0.7439 
